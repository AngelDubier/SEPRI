  ...
  return (
    <div className="container mx-auto px-4 py-12 min-h-screen">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {newsList.map((news) => (
          <article key={news.id} className="bg-white rounded-xl overflow-hidden shadow-md hover:shadow-xl transition-shadow duration-300 border border-gray-100 flex flex-col">
            ...
            <div className="p-6 flex-1 flex flex-col">
              ...
              <h2 className="text-xl font-bold text-sepri-dark mb-4">
                {news.title}
              </h2>
              <div className="text-gray-600 text-sm leading-relaxed mb-6 flex-grow">
                {renderFormattedText(news.summary)}
              </div>
        ...
        {newsList.length === 0 && (
          <div className="col-span-full py-20 text-center text-gray-500 italic">No hay comunicados publicados.</div>
        )}
      </div>
    </div>
  );
